...
import { getNotificationTarget } from '../../features/notifications/notificationLinks';
...
const NAV_ITEMS = [
  ...
  // Feed & Search are not implemented yet — shown disabled until they ship.
  { label: 'Feed',          to: '/feed',          Icon: IconFeedNav,       end: false, requiresAuth: true, disabled: true },
  { label: 'Search',        to: '/search',        Icon: IconSearch,        end: false, requiresAuth: true, disabled: true },
  ...
  return (
    <div className="min-h-svh bg-[var(--color-bg)] text-[var(--color-text-primary)]">
      ...
      <div className="max-w-[1440px] mx-auto flex min-h-svh lg:min-h-0">
        ...
        {/* ── Left: Navigation ── */}
        <aside className={['sidebar-scroll sidebar-scroll-chain hidden lg:flex flex-col w-[320px] shrink-0 sticky top-0 h-svh overflow-y-auto border-r border-[var(--color-border)] px-4 py-6', isGuest ? 'pb-20' : ''].join(' ')}>
        ...
        <main className={['flex-1 min-w-0 border-r border-[var(--color-border)]', isGuest ? 'pb-24' : ''].join(' ')}>
  ...
  return (
    <nav className="flex flex-col gap-0.5">
      {NAV_ITEMS.filter(item => !(item.authOnly && isGuest)).map((item) => {
        ...
        const { label, end, requiresAuth, disabled } = item;
        ...
        if (disabled) {
          return (
            <button
              ...
              disabled
              title="Coming soon"
              className="flex items-center gap-3.5 px-3 py-2.5 rounded-[var(--radius-md)] text-sm font-medium text-[var(--color-text-secondary)] opacity-40 cursor-not-allowed text-left w-full"
            >
...
function NotificationBubble({ ref, placement, isDesktop, position, onClose }) {
